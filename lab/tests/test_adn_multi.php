...
//require_once __DIR__ .'/../../core/src/Cipherenc3D.php';
require_once __DIR__ . '/../../core/src/Cipher.php';
require_once __DIR__ . '/../../core/src/Utils.php';
...
foreach ($files as $size => $inputFile) {
    echo "\n=== TEST $size ===\n";
    $encryptedFile = str_replace('.txt', '_8_32_2048_lancerblks64.mbk', $inputFile);
    ...
    $profile = $cipher->getProfile();

    echo "\n===== PROFILING =====\n";

    $total = 0;

    foreach ($profile['sections'] as $k => $v) {
        $total += $v;
        echo str_pad($k, 20) . " : " . round($v, 4) . " s\n";
    }

    echo "----------------------\n";
    
    echo "TOTAL sections     : " . round($total, 4) . " s\n";
    echo "DeCube calls      : " . $cipher->profile['deCube_calls'] . "\n"; 


}
if (method_exists($cipher, 'dumpProfile')) {
    $cipher->dumpProfile();
}
